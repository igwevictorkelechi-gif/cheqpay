...
export default function WithdrawAmountPage() {
  ...
  const valid = value >= 100 && !overBalance;
  ...
  return (
    <div className="flex min-h-screen justify-center bg-black">
      <div className="relative flex min-h-screen w-full max-w-[480px] flex-col bg-surface px-5 pb-6 pt-3">
        ...
        <p className={`mt-3 text-sm ${overBalance ? "text-red-400" : "text-muted"}`}>
          {overBalance
        ...
        <div className="mt-3 flex items-center gap-4 rounded-3xl bg-card p-5">
          ...
          <div>
            ...
            <p className="mt-0.5 text-sm text-muted">150 NGN fee · Arrives in seconds</p>
          </div>
        </div>
        ...
        <div className="mt-auto pt-6">
          <button
            disabled={!valid}
            onClick={() => router.push(`/withdraw/beneficiary?amount=${digits}`)}
